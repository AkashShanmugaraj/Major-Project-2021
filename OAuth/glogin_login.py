...
def theoauth(flaskapp = Flask(__name__)):
    ...
    def authorize():
        ...
        # Getting username with a defined function
        uname = getusername()

        # Saving credentials locally
        savecred(uname, user_info['name'])

        # Saving credentials to server
        cur.execute(f"INSERT INTO credentials (email,username,medium) VALUES ('{user_info['email']}','{uname}','Google');")
        db.commit()
...
def getusername():
    ...
    print('Choose a Username for yourself')
    uname = input()

    while uname in usernames:
        print('That one already exists')
