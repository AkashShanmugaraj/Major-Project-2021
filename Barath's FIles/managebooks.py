...
def add_book():
    ...
    if q == 'y':
        ...
        bstock = integernavigation('Enter Total Stocks of the Book: ')
        bprice = integernavigation('Name your price for the book: ')
        cur.execute(f"insert into books2 values('{bid}','{bname}',{isbn},'{author}','{category}','{publisher}', '{pubdate}', {bstock},{bprice},'False');")
        db.commit()
...
def viewbooks():
    ...
    .....'''
    selection = integervaluecontrol([1,2,3,4,5], menu2, 'Choose only anyone of the following (1,2,3)!!')
    if selection == 1:
